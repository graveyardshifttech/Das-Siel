"""
SielStore — Streaming .siel file format.
Loads ONLY metadata, streams concepts on demand.
Inspired by GGUF — no more 400MB RAM usage!
"""
...
import shutil
import numpy as np
from typing import Dict, Optional, List, Any, Set, Generator, Tuple
...
class SielStreamingStore:
    ...
    def stream_concepts(self, concept_type: Optional[str] = None) -> Generator[Dict, None, None]:
        """Stream concepts lazily."""
        with open(self.filename, 'rb') as f:
            f.seek(self.HEADER_SIZE + self.data_size + 4)  # Skip metadata size
            
            for concept_id in range(self.concept_count):
                try:
                    concept_data = self.read_concept(concept_id)
                    ...
                    if concept_type is None or concept['metadata'].get('type') == concept_type:
                        yield concept
                except:
                    continue
